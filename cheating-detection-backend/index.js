...
mongoose
  .connect(
    "mongodb+srv://radhita09:KOBi57TYbGZR1fQK@cluster0.ev3mw.mongodb.net/cheating-detection?retryWrites=true&w=majority&appName=Cluster0"
  )
...
io.on("connection", (socket) => {
    ...
    try {
      ...
      if (room) {
        ...
        if (room.hostSocketId) {
          ...
          console.log(
            `User ${userName} joined room ${roomCode}, notified host.`
          );
        } else {
  ...
  socket.on("cheating_detected", async (data) => {
    console.log(
      `Student ${data.userName} is suspected cheating, Room: ${data.roomCode}`
    );
    ...
    try {
      ...
      if (student) {
        ...
        if (room) {
          console.log(
            `Emitting cheating log to hostSocketId: ${room.hostSocketId}`
          );
          io.to(room.hostSocketId).emit("cheating_log", {
            logMessage: `${
              data.userName
            } is suspected cheating at ${formatTime()}`,
          });
        }
      } else {
  ...
  // Event for camera blocked detection
  socket.on("camera_blocked", async (data) => {
    console.log(
      `Student ${data.userName}'s camera is blocked in Room: ${data.roomCode}`
    );
    ...
    try {
      ...
      if (student) {
        const cheatingLog = new CheatingLog({
          student: student._id,
          roomCode: data.roomCode,
          behavior: "Camera blocked",
        });
        await cheatingLog.save();

        // Find the room and emit the camera block log to the host
        const room = await Room.findOne({ roomCode: data.roomCode });
        if (room) {
          console.log(
            `Emitting camera block log to hostSocketId: ${room.hostSocketId}`
          );
          io.to(room.hostSocketId).emit("camera_blocked_log", {
            logMessage: `${
              data.userName
            }'s camera is blocked at ${formatTime()}`,
          });
        }
      } else {
        console.error("Student not found in the database.");
      ...
      console.error("Error saving camera block incident:", error);
    }
  });
